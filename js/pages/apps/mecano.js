/**
 * mecano.js - Application "Atelier Mécanique"
 * Totalement indépendante. Visible uniquement par les membres ayant la
 * permission 'view' sur le module 'mecano' (via une fonction comme
 * "Mécanicien"/"Gérant Mécano", ou un grade).
 */
...
const TABS = [
    ...
    { key: 'services', label: '🛠️ Prestations' },
];

export async function mountMecanoApp(container, memberProfile) {
    ...
    async function show(key) {
        ...
        if (key === 'services') return renderServices(body);
    }
...
async function renderTreasury(body, memberProfile) {
    renderCrudApp(body, {
        moduleKey: 'mecano',
        table: 'v_mecano_treasury_full',
        baseTable: 'mecano_treasury_transactions',
        titleField: 'comment',
        orderBy: { column: 'created_at', ascending: false },
        emptyLabel: 'Aucune transaction de l\'atelier enregistrée.',
        statsRenderer: (data) => [
            el('div', { class: 'stat-card' }, [el('div', { class: 'stat-label' }, 'Solde atelier'), el('div', { class: 'stat-value' }, formatAmount(data[0]?.balance_after ?? 0))]),
        ],
...
async function renderStock(body) {
    const canCreate = can('mecano', 'create');
    const { data: overview } = await listRecords('v_mecano_stock_overview', { orderBy: { column: 'name', ascending: true } });
    ...
    const table = el('table', { class: 'app-table' }, [
        el('thead', {}, [el('tr', {}, ['Pièce / matériau', 'Quantité', 'Seuil', 'État'].map((h) => el('th', {}, h)))]),
    ...
    if (canCreate) {
        ...
        const btn = el('button', { class: 'btn btn-primary', onclick: async () => {
            await createRecord('mecano_stock_movements', { category_id: categorySelect.value, type: typeSelect.value, quantity: Number(qtyInput.value), comment: commentInput.value || null });
            body.innerHTML = '';
            renderStock(body);
        } }, '+ Mouvement');
...
async function renderInvoices(body, memberProfile) {
    renderCrudApp(body, {
        moduleKey: 'mecano',
        table: 'mecano_invoices',
        titleField: 'client_name',
        searchColumns: ['client_name', 'vehicle'],
        orderBy: { column: 'created_at', ascending: false },
...
async function renderServices(body) {
    renderCrudApp(body, {
        moduleKey: 'mecano',
        table: 'mecano_services',
        titleField: 'name',
        searchColumns: ['name'],
        orderBy: { column: 'name', ascending: true },
        emptyLabel: 'Aucune prestation définie.',
        columns: [
            { key: 'name', label: 'Prestation' },
            { key: 'price', label: 'Prix', render: (r) => formatAmount(r.price) },
            { key: 'description', label: 'Description' },
        ],
        fields: [
            { key: 'name', label: 'Nom de la prestation', required: true },
